...
func ArticlesKeyboard(catID int64, offset, pageSize int, arts []db.Article) tgbotapi.InlineKeyboardMarkup {
	...
	for _, a := range arts {
		rows = append(rows, tgbotapi.NewInlineKeyboardRow(tgbotapi.NewInlineKeyboardButtonData("📄 "+a.Title, cbArtPrefix+fmt.Sprint(a.ID))))
	}
	...
	if offset > 0 {
		...
		nav = append(nav, tgbotapi.NewInlineKeyboardButtonData("⬅️", cbCatPagePref+fmt.Sprintf("%d:%d", catID, prev)))
	}
	if len(arts) == pageSize {
		nav = append(nav, tgbotapi.NewInlineKeyboardButtonData("➡️", cbCatPagePref+fmt.Sprintf("%d:%d", catID, offset+pageSize)))
	}
	if len(nav) > 0 {
		rows = append(rows, tgbotapi.NewInlineKeyboardRow(nav...))
	}
	rows = append(rows, tgbotapi.NewInlineKeyboardRow(tgbotapi.NewInlineKeyboardButtonData("🏠 Категорії", cbHome)))
	return tgbotapi.NewInlineKeyboardMarkup(rows...)
}

func BackToCategoriesKeyboard() tgbotapi.InlineKeyboardMarkup {
	return tgbotapi.NewInlineKeyboardMarkup(tgbotapi.NewInlineKeyboardRow(tgbotapi.NewInlineKeyboardButtonData("🏠 Категорії", cbHome)))
}
